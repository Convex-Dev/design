...
The following CVM types convert directly to JSON:
...
### CVX to JSON Conversion Rules

CVX to JSON translation is:
- **complete** - any CVX value can be mapped to JSON
- **lossy** - some type information is lost

When mapping CVX format to JSON, implementations SHOULD adhere to the following egneral rules:

- **Keywords**: CVM `Keyword` values (e.g. `:foo`) are converted to JSON strings by removing the leading colon (e.g. `"foo"`)
- **Map Keys**: `Keyword`, `Symbol` and `String` keys in Maps are JSON-compatible. Other types are converted to String keys.
- **Blobs**: Converted to hex strings. A leading "0x" is recommended.
- **Lists/Vectors**: Converted to JSON arrays. The distinction between lists and vectors is lost
- **Numbers**: Large integers beyond JavaScript's safe integer range may lose precision when converted to JSON numbers
...
- Avoid CVM-specific types like `Address`, `Blob`, `Symbol`, or `Set` if JSON conversion is required
